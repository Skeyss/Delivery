            ...
            try
            {
                ...
                if (HasErrors == false)
                {
                    ...
                    if (service.Status)
                    {
                        ...
                        var inicioDeSesion = await Services.PersonaService.IncioDeSesionAsync(personaAGuardar.Telefono, personaAGuardar.Password);

                        if (inicioDeSesion.Status)
                        {
                            //login automatico
                            await Application.Current.MainPage.Navigation.PushModalAsync(new VerificacionCodigoPage(personaAGuardar.Telefono));
